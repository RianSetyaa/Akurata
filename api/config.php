...
function db(): PDO
{
    ...
    $host = getenv('DB_HOST') ?: ($localSecrets['DB_HOST'] ?? '127.0.0.1');
    $port = getenv('DB_PORT') ?: ($localSecrets['DB_PORT'] ?? '3306');
    $name = getenv('DB_NAME') ?: ($localSecrets['DB_NAME'] ?? 'akurata_erp');
    $user = getenv('DB_USER') ?: ($localSecrets['DB_USER'] ?? 'root');
    $pass = getenv('DB_PASS') ?: ($localSecrets['DB_PASS'] ?? '');
...
function loyverse_config(): array
{
    ...
    return [
        'client_id' => getenv('LOYVERSE_CLIENT_ID') ?: ($localSecrets['LOYVERSE_CLIENT_ID'] ?? ''),
        'client_secret' => getenv('LOYVERSE_CLIENT_SECRET') ?: ($localSecrets['LOYVERSE_CLIENT_SECRET'] ?? ''),
...
function brevo_config(): array
{
    ...
    return [
        'api_key' => getenv('BREVO_API_KEY') ?: ($localSecrets['BREVO_API_KEY'] ?? ''),
